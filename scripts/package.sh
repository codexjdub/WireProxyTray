...
extras=()
for item in LICENSE CHANGELOG.md RELEASE.md RELEASE.txt .github .gitignore; do [[ ! -e $item ]] || extras+=("$item"); done
...
tar --sort=name --owner=0 --group=0 --numeric-owner --mtime="@${SOURCE_DATE_EPOCH:-0}" \
    ...
    bin libexec licenses scripts packaging README.md VALIDATION.md Makefile go.mod go.sum internal cmd tests "${extras[@]}" \
    | gzip -n >"dist/$archive"
